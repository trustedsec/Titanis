...
namespace Titanis.Security.Kerberos.Test
{
	static class TestData_
	{
		...
		internal static KerberosCredential Cred_NoPreauth = new KerberosPasswordCredential(new UserPrincipalName(
			"NoPreauthTestUser",
			Realm),
			"password");
		internal static KerberosCredential Cred_TestUser = new KerberosPasswordCredential(new UserPrincipalName(
			"TestUser",
			Realm),
			"password");
		internal static SimpleKdcLocator locator = new SimpleKdcLocator(new IPEndPoint(IPAddress.Parse("192.168.66.15"), 88));

		internal static KerberosKeyCredential Cred_Krbtgt = new KerberosKeyCredential(new UserPrincipalName(
			"krbtgt",
			Realm),
		...
		//	);
		internal static KerberosCredential Cred_Workstation = new KerberosKeyCredential(new UserPrincipalName(
			"DESKTOP-BQQ7ARR",
			Realm),
			EType.Aes256CtsHmacSha1_96,
