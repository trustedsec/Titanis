namespace ms_pac
{
	...
	public struct KERB_VALIDATION_INFO : Titanis.DceRpc.IRpcFixedStruct
	{
		public void Encode(Titanis.DceRpc.IRpcEncoder encoder)
		{
			...
			encoder.WriteValue(this.SubAuthStatus);
			this.LastSuccessfulLogon.Encode(encoder);
			this.LastFailedLogon.Encode(encoder);

			if ((this.Reserved3_ == null))
			{
				this.Reserved3_ = new uint[2];
			}
			for (int i = 0; (i < 2); i++
			)
			{
				uint elem_0 = this.Reserved3_[i];
				encoder.WriteValue(elem_0);
			}
		...
		public void Decode(Titanis.DceRpc.IRpcDecoder decoder)
		{
			...
			this.SubAuthStatus = decoder.ReadUInt32();
			this.LastSuccessfulLogon.Decode(decoder);
			this.LastFailedLogon.Decode(decoder);
			if ((this.Reserved3_ == null))
			{
				this.Reserved3_ = new uint[2];
			}
		...
		public ms_dtyp.FILETIME LastSuccessfulLogon;
		public ms_dtyp.FILETIME LastFailedLogon;
		public uint[] Reserved3_;
