...
namespace Wmi;
internal abstract class WmiCommandBase : Command
{
	...
	protected async Task<int> ConnectAndRun(string serverName, CancellationToken cancellationToken)
	{
		...
		DcomClient dcom = await DcomClient.ConnectTo(serverName, rpcClient, cancellationToken, callback: new DcomLogger(this.Log));
